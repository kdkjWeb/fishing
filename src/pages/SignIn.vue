...
export default {
    data(){
        return {
            ...
            tableList: [   //表格的头部配置
                ...
                {prop: 'cdate', label: '签到时间', width: '160', align: 'right'},
    ...
    methods: {
        //获取所有点赞列表
        getAllSignList(pageSize,pageNum){
            this.$post('sign/managerGetSignList',{
                pageSize: pageSize ? pageSize : 30,
                pageNum: pageNum ? pageNum : 1,
                userId: this.formInline.userId ? this.formInline.userId : null,
                ctime: this.formInline.date ? `${this.dataTransform(this.formInline.date[0])} 00:00:00` : null,
        ...
        exportd(){
            ...
            let href = path + 'sign/downloadSignList'
            let json = {};
            ...
            Object.keys(this.formInline).forEach((key,index)=>{
                if(this.formInline[key] != '' && key != 'date'){
                    ...
                    json.ctime =  `${this.dataTransform(this.formInline.date[0])} 00:00:00`;
     ...
     handleSizeChange(val) {
            this.pageSize = val;
            this.getAllSignList(val,this.currentPage);
        },
        //当前第几页
      handleCurrentChange(val) {
            this.currentPage = val;
            this.getAllSignList(this.pageSize,val)
        },
        //标准时间格式转换
      dataTransform(date){
        if(date){
